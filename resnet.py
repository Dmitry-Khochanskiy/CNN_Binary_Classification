...
# coding: utf-8
'''
The module contains an untrained resnet architecture from Niko Gamulin with modified head classifier for binary classification
Args:
	Network depth (resnet 50, 101 etc)
Returns: An untrained resnet model
'''
...
class ResNet(nn.Module):
    def __init__(self, num_layers, block, image_channels, num_classes):
        ...
        self.fc = nn.Sequential(nn.Linear(512 * self.expansion, 1, bias = True),nn.Sigmoid())
...
def ResNet34(img_channels=3, num_classes=2):
...
def test():
    net = ResNet18(img_channels=3, num_classes=2)
